...
void loop() {
  ...
  for (idx = 0; idx < 4; idx++) {  //if servo position is valid, store to last known good, if invalid, load last known good instead
    if (servopos[idx] > 0 && servopos[idx] < 181) { servold[idx] = servopos[idx]; }
